...
1. **Dynamic Layer Probing:** Uniformly samples hidden states from any LLM architecture (Mistral, LLaMA, Qwen).
...
Run the complete 4-stage pipeline (feature extraction, deep training, and ensemble meta-learning):

```bash
# Detect hallucinations in HaluEval using Mistral-7B
python run_pipeline.py --dataset halueval --model mistral-7b

# Detect hallucinations in TriviaQA using LLaMA2-7B
python run_pipeline.py --dataset triviaqa --model llama2-7b
```
...
├── models/
│   ├── multihaludet.py    # HybridDeepLAP Architecture
